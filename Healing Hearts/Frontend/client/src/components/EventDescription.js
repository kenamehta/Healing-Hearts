...
onEventRegistered = (eventId,major) =>{
    ...
    render() { 
        return ( 
            ...
            <p> Registration closes on {this.props.eventdata?this.props.eventdata.date.split("T")[0]:''}</p>
           {console.log(this.props.eventdata.isRegister)}
           {
            ...
            {this.state.applysuccess?<p style={{color:'green'}}>Register successfully</p>:''}
               {this.state.noteligible?<p style={{color:'red'}}>{this.state.noteligible}</p>:''}
                ...
                <button className='form-control mt-2 btn btn-outline-success' onClick={(e)=>this.onEventRegistered(this.props.eventdata._id,this.props.eventdata.eligibility)} >Register</button>
                </div>
